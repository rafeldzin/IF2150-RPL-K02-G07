...
| *UC06* | *Menelusuri Daftar Produk* | *Pemilik usaha dapat mencari dan melihat produk berdasarkan nama dan tag yang diinput* | *Pemilik Usaha* | *KF15, KF16* |
...
| *UC09* | *Menerima Notifikasi Kerugian Produk* | *Pemilih usaha dapat mendapatkan peringatan ketika produk tertentu mengalami kerugian akibat perubahan harga bahan baku* | *Pemilik Usaha* | *KF21* |
...
| No | Aksi Aktor | Reaksi Perangkat Lunak |
| :--- | :--- | :--- |
| 1 | *Pemilik Usaha mengetik keyword nama atau tag yang ingin dicari* | *Sistem mencari semua produk dengan nama atau tag yang relevan* |
...
| No | Aksi Aktor | Reaksi Perangkat Lunak |
| :--- | :--- | :--- |
| 1 | *Pemilik Usaha memilih nama atau tag yang ingin dicari* | *Sistem mencari semua produk dengan nama atau tag yang relevan* |
...
| :--- | :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan seluruh data milik akun pemilik usaha berupa username, email, password, dan hal yang berkaitan dengan produk pemilik usaha (Entity Class).* | *UC01, UC02, UC03, UC04, UC05, UC06, UC07, UC08, UC09* |
| *C02* | *Produk* | *Menyimpan data produk berupa nama, harga jual, margin, dan tag yang dikelola oleh pemilik usaha (Entity Class).* | *UC03, UC04, UC05, UC06, UC07, UC09* |
| *C03* | *BahanBaku* | *Menyimpan data harga beli bahan baku yang digunakan sebagai resep produk (Entity Class).* | *UC03, UC04, UC08, UC09* |
...
| *C17* | *Notifikasi* | *Menyimpan detail pesan peringatan kerugian, waktu dikirim, dan status keterbacaan (Entity Class).* | *UC09* |
| *C18* | *PushNotificationUI* | *Antarmuka notifikasi sistem operasi (OS) yang muncul di perangkat pengguna (Boundary Class).* | *UC09* |
| *C19* | *NotificationController* | *Mendeteksi hasil margin negatif setelah pembaruan harga, membuat objek notifikasi, dan memicu pengiriman pesan (Controller Class).* | *UC09* |
...
| *C05* | *RegistrationPage* | *Antarmuka untuk menerima input data Username, Email dan Password pemilik usaha serta tombol "Buat Akun" untuk mendaftarkan akun pemilik usaha ke dalam database milik perangkat lunak (Boundary Class).* |
| *C06* | *AuthController* | *Memastikan bahwa input yang dilakukan oleh pemilik usaha sudah sesuai dengan data milik perangkat lunak (Controller).* |
...
| *C02* | *RegistrationPage* | *usernameInput, emailInput, passwordInput,* | *submitForm(), displaySuccess(), displayError()* |
| *C03* | *AuthController* | *-* | *validateData()* |
...
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang sedang membuat produk baru (Entity Class).* |
| *C07* | *LoginPage* | *Antarmuka untuk menerima input data Username/Email dan Password pemilik usaha serta tombol "Login" (Boundary Class).* |
| *C06* | *AuthController* | *Memastikan bahwa input yang dilakukan oleh pemilik usaha sudah sesuai dengan data milik perangkat lunak (Controller).* |
...
| *C02* | *LoginPage* | *usernameInput, passwordInput,* | *login(), navigateToResetPassword(), displayError()* |
| *C03* | *AuthController* | *-* | *validateData()* |
| *C04* | *ResetPasswordPage* | *emailInput, newPasswordInput, confirmNewPassword* | *submitResetRequest(), submitNewPassword(), displayError(), displaySuccess()* |
| *C05* | *ResetPasswordController* | *-* | *validateData(), resetPassword()* |
...
| ID Kelas | Nama Kelas | Deskripsi Kelas |
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang sedang menambahkan produk baru (Entity Class).* |
| *C02* | *Produk* | *Menyimpan data entitas produk baru yang diinput oleh pengguna, meliputi nama, label/tag, harga jual, dan margin (Entity Class).* |
| *C03* | *BahanBaku* | *Menyimpan data entitas bahan baku yang dipilih pengguna sebagai komposisi resep produk (Entity Class).* |
| *C04* | *ResepBahan* | *Menyimpan data entitas takaran spesifik dari bahan baku yang digunakan untuk menyusun resep suatu produk (Entity Class).* |
| *C10* | *AddProductPage* | *Antarmuka untuk menerima input data produk baru, bahan baku, takaran, harga jual, serta menampilkan hasil kalkulasi (Boundary Class).* |
| *C11* | *MarginController* | *Memvalidasi nama produk, memeriksa input takaran/harga jual, memproses kalkulasi margin, serta menyimpan data produk dan resep ke database (Controller Class).* |
...
| *C02* | *Produk* | *namaProduk, label, hargaJual, margin* | *getNamaProduk(), setNamaProduk(), getLabel(), setLabel(), setHargaJual(), setMargin(), getMargin()* |
| *C03* | *BahanBaku* | *namaBahan, hargaBeli* | *getNamaBahan(), getHargaBeli()* |
| *C04* | *ResepBahan* | *takaran* | *getTakaran(), setTakaran()* |
| *C10* | *AddProductPage* | *inputNamaProduk, inputLabel, inputTakaran, inputHargaJual* | *showPage(), getInput(), showErrorMessage(), showSuccessMessage()* |
...
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang sedang membuat produk baru (Entity Class).* |
| *C03* | *BahanBaku* | *Menyediakan data referensi berupa informasi harga beli standar bahan dari pasar, yang diperlukan oleh sistem untuk mengalkulasi harga jual standar (Entity Class).* |
| *C02* | *Produk* | *Menyimpan data entitas produk yang sedang diinput oleh pengguna, meliputi informasi nama, tag, harga jual yang ditetapkan, serta nilai akhir marginnya. (Entity Class).* |
...
| *C10* | *AddProductPage* | *Antarmuka untuk menerima input data produk dan menampilkan hasil kalkulasi secara real time (Boundary Class).* |
| *C11* | *MarginController* | *Menghitung perbedaan harga jual dan harga standar untuk menghasilkan margin yang untung/rugi (Controller Class).* |


#### Diagram Kelas

<p align="center">
<img alt="Class Diagram UC01" src="./assets/diagram/Diagram-UC04.webp" width="70%">
</p>
<p align="center">
...
| *C20* | *RiwayatHargaJual* | *Menyimpan data entitas catatan harga jual lama dari produk sebelum nilainya diperbarui (Entity Class).* |
| *C12* | *EditProductPage* | *Antarmuka untuk meginput data harga jual baru (Boundary Class).* |
| *C13* | *PriceController* | *Memvalidasi input, baik positif maupun negatif, memproses perubahan, dan menciptakan history (Controller Class).* |

#### Diagram Kelas

<p align="center">
<img alt="Class Diagram UC01" src="./assets/diagram/Diagram-UC05.webp" width="70%">
</p>
<p align="center">
...
| ID Kelas | Nama Kelas | Deskripsi Kelas |
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang sedang menelusuri daftar produknya (Entity Class).* |
| *C02* | *Produk* | *Menyimpan data entitas produk, meliputi nama, tag, harga jual, dan margin, yang menjadi target pencarian (Entity Class).* |
| *C21* | *SearchProductPage* | *Antarmuka untuk menerima input kata kunci/tag pencarian dari pengguna dan menampilkan daftar produk hasil pencarian (Boundary Class).* |
| *C22* | *ProductSearchController* | *Memproses kata kunci/tag yang diinput dan melakukan pencocokan (filtering) terhadap data produk untuk menghasilkan daftar produk yang relevan (Controller Class).* |

#### Diagram Kelas

<p align="center">
<img src="assets/diagram/image_uc06.png" width="70%">
</p>
<p align="center">
...
| ID Kelas | Nama Kelas | Deskripsi Kelas |
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang sedang melihat riwayat perubahan harga jual produknya (Entity Class).* |
| *C02* | *Produk* | *Menyimpan data entitas produk yang harga jualnya ingin ditelusuri riwayat perubahannya (Entity Class).* |
| *C20* | *RiwayatHargaJual* | *Menyimpan data entitas catatan perubahan harga jual suatu produk beserta waktu perubahannya (Entity Class).* |
| *C23* | *PriceHistoryPage* | *Antarmuka untuk menampilkan data/grafik historis perubahan harga jual suatu produk (Boundary Class).* |
| *C24* | *PriceHistoryController* | *Mengambil data riwayat harga dari database dan menyusunnya menjadi format yang dapat ditampilkan (Controller Class).* |

#### Diagram Kelas

<p align="center">
<img src="assets/diagram/image_uc07.png" width="70%">
</p>
<p align="center">
...
| ID Kelas | Nama Kelas | Deskripsi Kelas |
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang sedang memantau tren harga (Entity Class).* |
| *C03* | *BahanBaku* | *Menyimpan data entitas bahan baku yang akan dilihat riwayat perubahannya (Entity Class).* |
| *C14* | *RiwayatHargaBahanBaku* | *Menyimpan data entitas catatan fluktuasi harga beli bahan baku beserta waktu perubahannya (Entity Class).* |
| *C15* | *TrenBahanBakuPage* | *Antarmuka yang menampilkan daftar bahan baku, rentang waktu, dan memvisualisasikan grafik tren (Boundary Class).* |
| *C16* | *TrendController* | *Mengambil data riwayat harga dari database, menerapkan filter waktu, dan mengonversinya menjadi format grafik (Controller Class).* |

#### Diagram Kelas


<p align="center">
<img src="assets/diagram/image2.png" width="70%">
</p>
<p align="center">
...
| ID Kelas | Nama Kelas | Deskripsi Kelas |
| :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *Menyimpan data entitas akun pemilik usaha yang menjadi target penerima notifikasi (Entity Class).* |
| *C02* | *Produk* | *Menyimpan data entitas produk yang terdampak kerugian, termasuk status peringatan aktifnya (Entity Class).* |
| *C03* | *BahanBaku* | *Menyimpan data bahan baku yang mengalami pembaruan harga beli sehingga memicu kalkulasi ulang (Entity Class).* |
| *C17* | *Notifikasi* | *Menyimpan detail pesan peringatan kerugian, waktu dikirim, dan status keterbacaan (Entity Class).* |
| *C18* | *PushNotificationUI* | *Antarmuka notifikasi sistem operasi (OS) yang muncul di perangkat pengguna (Boundary Class).* |
| *C19* | *NotificationController* | *Mendeteksi hasil margin negatif setelah pembaruan harga, membuat objek notifikasi, dan memicu pengiriman pesan (Controller Class).* |
...
| ID Kelas | Nama Kelas | Atribut | Metode/Operasi |
| :--- | :--- | :--- | :--- |
| *C01* | *PemilikUsaha* | *username, email, password* | *createUser(), getUsername(), getEmail(), getPassword(), setPassword()* |
| *C02* | *Produk* | *namaProduk, label, hargaJual, margin* | *getNamaProduk(), setNamaProduk(), getLabel(), setLabel(), setHargaJual(), setMargin(), getMargin()* |
| *C03* | *BahanBaku* | *namaBahan, hargaBeli* | *getNamaBahan(), getHargaBeli()* |
| *C04* | *ResepBahan* | *takaran.* | *getTakaran(), setTakaran()* |
...
| *C14* | *RiwayatHargaBahanBaku* | *hargaBeliLama, waktuPerubahan* | *getHargaBeliLama(), getWaktuPerubahan()* |
| *C15* | *TrenBahanBakuPage* | *bahanBakuTerpilih, rentangWaktuFilter* | *showPage(), showChart(), showErrorMessage()* |
| *C16* | *TrendController* | *McurrentPemilik, currentBahanBaku* | *getRiwayatHarga(), filterDataByDate(), generateChartData()* |
| *C17* | *Notifikasi* | *idNotifikasi, pesanAlert, waktuKirim, isRead* | *getPesanAlert(), markAsRead()* |
| *C18* | *PushNotificationUI* | *notifikasiAktif* | *tampilkanNotifikasi(), onClickNotification()* |
| *C19* | *NotificationController* | *currentPemilik, targetProduk* | *cekMarginNegatif(), buatNotifikasiKerugian(), kirimNotifikasi()* |
...
| :--- | :--- | :--- |
| *C01* | *UC01, UC02, UC03, UC04, UC05, UC06, UC07, UC08, UC09* | *KF01, KF02, KF03, KF04, KF05, KF06, KF07, KF08, KF09, KF10, KF11, KF12, KF13, KF14, KF15, KF16, KF17, KF18, KF19, KF20, KF21* |
